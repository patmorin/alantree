...
Applying \lemref{meta-theorem} gives the following corollary of
\thmref{bestgrid-universal}.
...
The proof of \thmlabel{bestgrid-universal} is broken up into two parts,
...
In particular, we are done if $k/\ln(k+1) \geq d^{2/3}$.

To finish the proof of \thmref{bestgrid-universal} for the case where
the dimension is large, we will make use of a concentration result
about the diameter of the $d$-cube whose strength increases with $d$.
For a graph $G$, define
\[
    \diam(G) = \max\{\tau(G,s) : s\in V(G)\} \enspace .
\]

\begin{lem}\lemlabel{cubediameter}
   Let $G$ be the $d$-cube.  Then there exists universal constants
   $c$ and $x_0$ such that,
   \[ 
     \Pr\{\diam(G) > x\} \le e^{-cxd} \enspace ,
   \]
   for all $0 \le x \le 10d^2$.
\end{lem}
...
\begin{proof}
   ...
   Balister \etal\ \cite[Lemma~4]{balister.bollobas.ea:first}
...
of these cubes.  We upper-bound this sum using \lemref{cubediameter}
...
\lemref{cubediameter} to these $d$-cubes, but \lemref{cubediameter} has
a limited range of applicability, so we first rule out an unusual case.
For some $\alpha > 0$, let $\mathcal{E}$ denote the event
\[
    \max\{\diam(Q_u : u\in \{0,\ldots,k-1\}^d \} > \alpha d^2 \enspace .
\]
By \lemref{cubediameter} and the union bound,
\[
    \Pr\{\mathcal{E}\} \le k^de^{-10cd^2} = e^{d\ln k - \alpha cd^2}
\]
For any graph $Q$, define $\diam'(Q) = \min\{\alpha cd^2, \diam(Q)\}$
...
Let $X_1,\ldots,X_k$ and $Y_1,\ldots,Y_k$ be random variables
each distributed like $\diam'(Q_s)$ and such that $X_1,\ldots,X_k$
are independent and $Y_1,\ldots,Y_k$ are independent.  It follows
from the preceding discussion that $\sum_{i=1}^{k'}\diam'(Q_{v_i})$
is stochastically dominated by $\sum_{i=1}^k (X_i+Y_i)$.  We now use
...
\end{align*}
Letting $\tau'(G,s,v)=\max\{\alpha kd^2, \tau(G,s,v)\}$
and 
...
\begin{align*}
   \Pr\{\tau(G,s) > Ck\} 
     ...
     & \le o_n(1) + e^{\ln d + d\ln(k+1) - \alpha d^2} \\
     & \le o_n(1)
\end{align*}
for $\ln(k+1) < \alpha d/2$.  This last condition is certainly true
if $k/\ln(k+1) < d^{2/3}$.  
\end{proof}
