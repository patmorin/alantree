...
The proof of \thmref{bestgrid-universal} has two parts depending on the
relative values of $k$ and $d$.  Since it is long, and somewhat technical,
the details are relegated to \appref{grid-proof}.
...
\lemref{cube-diametrical} extends from $\Gamma(G,s,\bar{s}$ to $\Gamma(G,s)$ by an easy argument based on the fact that any vertex $v$ is at distance at least $d/2$ from at least one of $s$ or $\bar{s}$. (This argument is used by Bollob\'as \etal\ \cite{bollobas

\begin{cor}\corlabel{cube-radius}
   Let $G$ be the $d$-cube, and $s$ be any vertex of $G$.
   Then there exists universal constants
   $c>0$ and $x_0>0$ such that,
   \[ 
...
Observe that \corref{cube-diameter} essentially says that the tail of
$\diam(G)$ is dominated by an exponential random variable.  In particular,
if $X$ is $\exponential(cd)$, then
\[
    \Pr\{\diam(G) > x\} \le \Pr\{X > x\}
\]
for all $x>x_0$. Unsurprisingly, using this with Chernoff's moment-generating te this leads to a concentration result
similar to \eqref{tail-bound} for sums of exponentials.

\begin{lem}\lemlabel{cube-diameter-sum}
   Let $G$ be a $d$-cube and let $X_1,\ldots,X_k$ be i.i.d., random
   variables each distributed like $\diam(G)$.  Then there exists universal constants $c>0$
   and $x_0>0$ such that
   \[ \Pr\left\{\sum_{i=1}^k X_i > ak\right\} \le e^{(cdx_0/2+1)k-acdk/2} \]
\end{lem}
