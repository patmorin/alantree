...
\DeclareMathOperator{\erlang}{Gamma}
...
Let $D=D(G)$ and $\Delta=\Delta(G)$ denote the diameter and maximum degree
of $G$, respectively, and let us denote the height of a rooted tree $T$
by $h(T)$.  An obvious lower bound for $h(\tcal(G,s))$ is $D/2$.  We prove
the following bounds hold with probability $1-o_n(1)$ for any $n$-vertex
graph $G$ and any $s\in V(G)$.  (The notation $o_k(1)$ denotes the set
of functions $f:\R\to\R$ such that $f(k)\to 0$ as $k\to\infty$.)  In all
of these results, the reader should keep in mind that the diameter, $D$,
...
\begin{enumerate}
  \item In Theorem~\ref{thm:alantree-upper-bound} we show $h(\tcal(G,s))= O(\Delta(D+\log n))$.
  For $D= \Omega(\log \Delta)$ this is tight: in~\thmref{genericlowerbound} we show that for every $\Delta\ge  2$ and every $D \ge e^6\ln\Delta$, there exist $G$ and $s$ with 
   $h(\tcal(G,s))= \Omega(\Delta(D+\log n))$.

  \item If $G$ is $d$-degenerate (that is, every subgraph of $G$ has a vertex of degree at most $d$),
  then in Theorem~\ref{thm:alantree-upper-bound-degenerate} we show  $h(\tcal(G,s))=
...
For $D= \Omega(\log \Delta)$ and planar graphs (which are 5-degenerate) this is tight:
...
Also, for $D= \Omega(\log \Delta)$ and $d\leq\Delta$ this is tight:
...
$h(\tcal(G,s))=
  ...
  $h(\tcal(G,s))= O(\sqrt{\Delta}(D+\log n))$ 
  (see \thmref{alantree-upper-bound-genus})  .
  For $D= \Omega(\log \Delta)$ and zero Euler genus this is tight:
  in~\thmref{planartight} we show 
  for any $\Delta>2$ and $D>10^6\ln \Delta$
  ...
  $n=(k+1)^d$ vertices), we have $h(\tcal(G,s))= O(dk+d^{5/3}\ln(k+1))$.
  If $k=2$ or $k/\ln(k+1)=\Omega(d^{2/3})$, we have $h(\tcal(G,s))= \Theta(D)=\Theta(dk)$
  ...
  $h(\tcal(G,s))= O(\Phi^{-1}\Delta\log n)$ (see \thmref{alantree-upper-bound-cheeger}).  This implies, for example,
  that $h(\tcal(G,s))= O(\log n)$ if $G$ is the complete graph or if $G$ is
  a random $\Delta$-regular graph (since a random $\Delta$-regular
  graph has $\Phi= \Omega(\Delta)$, see~\cite{bollobas:isoperimetric}).
\end{enumerate}
...
Let $\Gamma(s,v)$ denote the set of all paths from $s$ to $v$ in $G$.
...
We use the following notational conventions: $\log_2 x$ denotes the
binary logarithm of $x$ and $\ln x$ denotes the natural logarithm of $x$.
Every graph, $G$, that we consider is finite, simple, undirected and
connected, and $n$ denotes its number of vertices.
...
for all $d>1$,
\begin{equation}
    \Pr\left\{\sum_{i=1}^k Z_i \le \mu k/d\right\} \le \exp\left(-k\left(\ln d -1 + \frac{1}{d}\right)\right) \le \left(\frac{e}{d}\right)^k  \eqlabel{head-bound}
\end{equation}
and for all $t\ge 2$, 
\begin{equation}
...
random variables is called a $\erlang(k,\lambda)$ random variable.
%distribution.
%, i.e.,
%\[
...
\begin{proof}
   Let $T = \tcal(G,s)$.
   If $h(T)> L$, then $T$ contains a root-to-leaf path of length greater
   than $L$ and therefore $T$ contains a path that begins at the root
   and has length exactly $L$.  This implies that at least one of the
...
\thmref{alantree-upper-bound} up to constant factors, since all graphs
of maximum degree $\Delta$ are $\Delta$-degenerate, so $\sqrt{d\Delta}\le
\Delta$ in all cases.  However, \thmref{alantree-upper-bound-degenerate}
provides sharper bounds for  many important graph classes:

\begin{itemize}
  \item Planar graphs are 5-degenerate. (This is a consequence of Euler's
    formula and the fact that planarity is preserved
    under taking subgraphs---see, for example,
    \cite[Corollary~10.22]{bondy.murty:graph}).
  
  \item The \emph{thickness} of a graph is the minimum number of planar
    graphs into which the edges of $G$ can be partitioned.  If $G$ has
    thickness $t$ then, by Euler's Formula, $G$ has $m<3tn$ edges and
    the average degree of $G$ is less than $2m/n < 6t$. It follows that
    graphs of thickness $t$ are $(6t-1)$-degenerate.
  ...
  \item The \emph{Euler genus} of a graph is the minimum Euler genus of
    ...
    Graphs of Euler genus $g$ are $O(\sqrt{g})$-degenerate.\footnote{In
    every $n$-vertex Euler-genus $g$ graph, $n\in \Omega(\sqrt{g})$
    and there exists a vertex of degree at most $6+O(g/n)$. (See, e.g.,
    \cite[Lemma~7 and Theorem~2]{wolle.koster.ea:note}.)}
...
when the relevant parameter, $g$, $t$ or $k$, is bounded, $h(T)=
...
$g$, then $h(\tcal(G,s))= O(g^{1/4}\Delta^{1/2}(D+\log n))$.  In this section we show that the
...
\begin{proof}
   ...
   is called \emph{bad} if it is annoying or if $i_{\ell-1}>i_{\ell}$;
   otherwise it is called good.  Let $k$ denote the number of good edges
   in $P$.  Say $P$ is good if $k \geq L/2-3g$.  Note that the number
   of annoying edges of $P$ is bounded by $6g-1$, hence at least one of
   $P$ and its reverse is good.  We bound the number of good $L$-paths;
   the total number of $L$-paths is at most twice this bound.  We encode
   a good $L$-path $P$ as follows:
   \begin{enumerate}
     \item We first specify the starting vertex $v_{i_0}$.  There are $n$
       ways to do this.
...
\begin{thm}\thmlabel{alantree-upper-bound-genus}
  ...
  If $g\ln\Delta \le 36\sqrt{\Delta}(D+\ln n)$ then, with probability
  at least $1-O(1/n)$, $h(\tcal(G,s))\le 107\sqrt{\Delta}(2D+4\ln n)$.
\end{thm}
...
dependence on $\Delta$, the maximum degree of a vertex in $G$.  This seems
somewhat counterintuitive, since high degree vertices in $G$ should
produce high degree vertices in $T$ and therefore decrease $h(T)$.
In this section we show that indeed large edge expansion (also called
isoperimetric number or Cheeger constant) results in low-height trees.
...
\begin{thm}\thmlabel{alantree-upper-bound-cheeger}
  ...
  $h(\tcal(G,s)) = O(\Psi\Delta)\subseteq O(\Phi^{-1}\Delta\log n)$.
\end{thm}
...
Then \thmref{alantree-upper-bound-edge-expander} implies that $h(\tcal(K_n,s))=
...
\lemref{few-walks} gives a better upper bound for $d$-degenerate graphs,
using which we immediately obtain the following corollary.
...
\begin{cor}
  ...
  $h(\tcal(G,s))= O(\Psi\sqrt{d\Delta}+\log n)
  = O(\log n (1+ \sqrt{d\Delta}/\Phi))$.
\end{cor}
...
bound can be extended to $k= O(1)$.   \thmref{alantree-upper-bound}
gives an upper bound of $O(d^2k)$ on $h(\tcal(G,s))$, which is optimal
for $d= O(1)$.  The rest of this section is devoted to proving the
...
\begin{proof}
   ...
   For $x > d^2$, let $r=2^{\lceil\log_2(x/d^2)\rceil}$, so that 
   \[ x_0 \le x/r \le d^2 \]
   ...
   Now, there are $d$ edge-disjoint paths of length at most $4$ joining
   $s$ to $u$ (see \figref{manypaths}).  Consider one such path, $P$.  If $P$
   has weight greater than $x/3$ then at least one of $P$'s edges has
   weight greater than $x/12$, which occurs with probability at most
   $4e^{-x/12}$.  Therefore, the probability that all $d$ paths have
   weight greater than $x/3$ is at most $(4e^{-x/12})^d \le e^{-adx}$ for
   $0<a<\ln 4/x - 1/12$.  Similarly, with probability at least $1-e^{-adx}$,
   there is a path of length at most 4 and weight at most $x/3$ joining
   $w$ to $\bar{s}$.
   \begin{figure}
      \begin{center}
         \includegraphics{figs/manypaths}
      \end{center}
      \caption{The $d$-cube contains $d$ edge disjoint paths of length
        at most 4 going from $s\in L_0$ to $u\in L_2$.}
      \figlabel{manypaths}
   \end{figure}
   ...
   \[
      \Pr\{\tau(Q,s,\bar{s}) > 2x/3 + d\ln(d/(d-a))\} 
        \le \Pr\{\tau(Q,s,\bar{s}) > x\}  
           \le 2e^{-axd}+e^{-\gamma d^2}
   \]
   provided that $x \ge\max\{3d\ln(d/(d-a)),\ln 4/(1/12-a)\}$. This holds, 
   for example, when $x\ge x_0 = 24\ln 4$, $a=1/24$, and $d\ge 1$.
\end{proof}
...
\begin{lem}\lemlabel{cube-diameter-sum}
   ...
   be independent random variables where $Z_i$ is distributed like
...
\begin{proof}
   ...
   $m-1$ $\exponential(\delta)$ random variables, i.e.\ a $\erlang(m-1,\delta)$ random variable.
...
The third inequality is because a $\erlang(L,\delta)$ random variable stochastically dominates a $\erlang(m-1,\delta)$ random variable,
...
where each $V_i$ has $\delta$ vertices
  ...
  $(V_1,V'_1)$, $(V'_1,V_2)$, $(V_2,V'_2)$, etc.\ forms a complete bipartite graph.
Let $I$ be the perfect binary tree with $L$ leaves,
...
discussions on many aspects of this work and to Tasos~Sidiropoulos for
asking us about planar graphs. The authors would also like to thank
David Wood for pointing out, and offering a fix for, a mistake in our
discussion of graph thickness.
