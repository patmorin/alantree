...
\begin{enumerate}
  ...
  See \corref{alantree-upper-bound-dk-grid}.
  ...
  See \thmref{alantree-upper-bound-cheeger}.
\end{enumerate}

Our main tool, \lemref{meta-theorem}, relates the quantity, $h(T)$, that
...
\thmref{alantree-lower-bound} shows that \thmref{alantree-upper-bound}
cannot be strengthened without knowing more about $G$ than its maximum
degree.  \thmref{alantree-upper-bound-degenerate} provides a stronger
...
%Recall the simple Chernoff Bound: For a $\binomial(n,p)$ random variable $B$,
%\begin{equation}
%   \Pr\{B \le np/2\} \le e^{-np/8} \enspace . \eqlabel{chernoff-a}
%\end{equation}
%\begin{lem}\lemlabel{easy-key}
%Let $\delta \ge d > 0$ be integers, and $0 < c \le \sqrt{\delta d}$, and let $T$ be a tree of height 2 whose root has $\delta$
%children and whose level 1 nodes each have $d$ children and whose edges
%are each assigned independent $\exponential(1)$ weights.  Then the
%probability that $T$ has no root-to-leaf path of weight less than
%$(c+1)/\sqrt{\delta d}$ is at most $e^{-c/4e} + e^{-2c/e}$.
%\end{lem}
%
%\begin{proof}
%   Let $X_1$ be the number of edges incident to the root that have weight at most $c/\sqrt{\delta d}$.  Then $X_1$ is a $\binomial(\delta, p)$ random variable with 
%\[
%    p=1-e^{-c/\sqrt{\delta d}} \ge \frac{2c}{e\sqrt{\delta d}} \enspace ,
%\]
%where the inequality uses the assumption that $c/\sqrt{\delta d} \le 1$.
%In particular from \eqref{chernoff-a},
%\[
%   \Pr\{X_1 < \delta p / 2 \} \le \exp\left(-\frac{\delta p}{8}\right) 
%   \le \exp\left(-\frac{c\sqrt{\delta}}{4e\sqrt{d}}\right) 
%   \le e^{-\frac{c}{4e}} \enspace .
%\]
%
%   Let $X_2$ denote the number of root-to-leaf paths in $T$ whose first
%   edge has weight at most $c/\sqrt{\delta d}$ and whose second edge has
%   weight at most $1/\sqrt{\delta d}$.  Then, conditional on $X_1$, $X_2$
%   is a $\binomial(d X_1, 1-e^{-1/\sqrt{\delta d}})$ random variable. Now we have
%\[
%   \Pr\{X_2 = 0\mid X_1 \ge \delta p/2\} \le
%   \left(e^{-\frac{1}{\sqrt{\delta d}}}\right)^{d\delta p/2}
%   \le e^{-2c/e} \enspace . \qedhere
%\]
%\end{proof}
...
From \lemref{tree-easy}, we can obtain an upper bound on the expected weight, $X$, of the lightest root-to-leaf path of in $T_\alpha,\beta,\gamma$. of the shortest path.  Indeed, setting $x=(c+2)q$, \lemref{tree-easy} implies that, for $0<x<1+2/q$,
\[
...
\begin{align*}
   \E[X] & = \int_0^{\infty} \Pr\{X > x\} \\
         ...
         & \le \int_0^{1} 3e^{-\left(\frac{x}{q}-2\right)/(4e^2)}\,\mathrm{d}x + \int_1^{\infty} \Pr\{X > x\}\,\mathrm{d}x \\
         & = 3e^{2/(4e^2)}\int_0^{1} e^{-x/(4e^2q)}\,\mathrm{d}x + \int_1^{\infty} \Pr\{X > x\}\,\mathrm{d}x \\
         & = 3e^{2/(4e^2)}4e^2q + \int_1^{\infty} \Pr\{X > x\}\,\mathrm{d}x \\
         & = 3e^{2/(4e^2)}4e^2(\alpha\beta\gamma)^{-1/3} + \int_1^{\infty} \Pr\{X > x\}\,\mathrm{d}x
\end{align*}
