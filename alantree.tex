...
Applying \lemref{meta-theorem} gives the following corollary of \thmref{bestgrid-universal}.
...
To prove \thmref{bestgrid-universal} we will make use of the following lemma
...
%   show that there is a constant $c>0$ such that, if we sample each edge
%   of $Q$ independently with probability $p=a/d\ge 6/d$ then, with probability
%   at least $1-e^{-cad^2}$, there is a path of length $d-4$ consisting
...
%   at most $\ln(d/(d-x)) = O(1/d)$, then, with probability at least
%   $1-e^{-d^2/800}$, there is a path of weight at most $d\ln(d/(d-6))$
%   joining a vertex $u$ in $L_2$ to a vertex $w$ in $L_{d-2}$.
%
%   Now, there are $d$ paths of length $2$ joining $s$ to $u$ and with
%   probability at least $1-2e^{-x d/6}$ at least one of these paths
%   has weight no more than $x/3$.  Similarly, with probability at least
%   $1-2e^{-xd/6}$ there is a path of length 2 of and weight at most $x/3$
%   joining $w$ to the unique vertex $\bar{s}$ in $L_d$.
%
...
%   \[
%          2x/3 + d\ln(d/(d-6))  \le x
%   \]
%   provided that $x \ge 3d\ln(d/(d-6))$.
%   
\end{proof}
...
\]
Now, taking $x_1=\max\{1,x_0\}$, where $x_0$ appears in \lemref{cubediameter}, 
we have
\begin{align*}
   \E\left[e^{tX_1}\right]
