...
\figref{clique-path}) shows that Process~B may produce spanning trees of
...
%  Note that $2^{h}>i-j-1$
%  \[
%     \Pr\{W_I \le 2a(j-i-1)/(\beta e^2)\} \le 
%     \Pr\{W_I \le 2a2^h/(\beta e^2)\} \le 
%     \Pr\{W_I \le 2s(h)/e^2\} \le L^{-2a} \enspace ,
%  \]
%  where the last inequality is the statement of \lemref{slow-train}.
%  So, $W_I$ is almost always longer than $2a(j-i-1)/(\beta e^2)$
  Let $r=s(h)/e^2$,
  \[
  ...
  \lemref{slow-train} and \lemref{lightning}, respectively.
...
%      t = \frac{\delta s(h)}{e^2(j-i-1)} \enspace . 
%  \]
%  Recall that $s(h)=\max\{a\log L,a2^h/\delta\}$.  When $s(h)=a\log L$, we have
%   
%  \begin{align*}
%     \Pr\{W_H > W_I\} & \le \exp((1-t/2)(j-i-1)) + \exp(-r) \\
%         & \le \exp(j-i-1-\delta a\log L/e^2) + \exp(-(a/e^2)\log L) \\
%         & = 2L^{-\Omega(a)}
%  \end{align*}
%  
%
%
%  If $s(h) = a 2^h/\delta$ (because $2^h/\delta\ge\log L$), then
%  \[   s(h)
%  then we obtain:
%  \begin{align*}
%     \Pr\{W_H > W_I\} & \le \exp((1-t/2)(j-i-1)) + \exp(-r) \\
...
%  \begin{align*}
%     \Pr\{W_H > W_I\} 
%         & \le \exp\left(j-i-1 - \frac{\delta a\log L}{2e^2}\right) + L^{-a/e^2} \\
